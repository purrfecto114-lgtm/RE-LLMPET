...
function renderCodexUsage(codexUsage) {
  ...
  $('codex-today-detail').textContent = t('panel.codexBreakdown', { in: fmt(today.input), out: fmt(today.output), cached: fmt(today.cached || 0), reasoning: fmt(today.reasoning || 0) });
  $('codex-lifetime-detail').textContent = t('panel.codexLocalHistory', { sessions: lifetime.sessions || 0, events: lifetime.events || 0 });
}
...
function renderDiagnostics(diag) {
  ...
  const last = diag.lastScanTs
    ...
    : t('panel.diagNever');
  const bits = [t('panel.diagScan', { when: last, files: diag.scannedFiles || 0, records: diag.records || 0 }), t('panel.diagCorrections', { n: diag.streamingCorrections || 0 })];
...
function renderProviderCost(providerCost) {
  ...
  for (const [id, v] of entries) {
    ...
    html += `<div class="row pcost-row"><span class="pcost-name">${escapeHtml(m.icon)} ${escapeHtml(m.label)}</span><span class="pcost-bar-wrap"><span class="pcost-bar" style="width:${pct}%"></span></span><b>${escapeHtml(costText)}</b><span class="pcost-sub">${fmt(v.tokens)} tok · ${v.messages || 0}${escapeHtml(t('panel.rounds'))}${unknownText ? ' · ' + unknownText : ''}</span></div>`;
  }
  el.innerHTML = html;
}
...
function renderByModel(byModel) {
  ...
  for (const [model, v] of entries) {
    ...
    const costText = aggregateCostText(v);
    html += `<div class="m-item"><div class="m-head"><span class="mc">${escapeHtml(shortModel(model))}</span><span class="m-bar"><i style="width:${pct}%"></i></span><b class="m-cost">${escapeHtml(costText)}</b><span class="m-tok">${fmt(v.tokens)} · ${pct}%</span></div>${detail}</div>`;
   }
   html += `<div class="m-item m-total"><div class="m-head"><span class="mc">${escapeHtml(t('panel.total'))}</span><span class="m-bar"></span><b class="m-cost">${escapeHtml(aggregateCostText({ cost: totCost, messages: entries.reduce((sum, [, value]) => sum + (Number(value.messages || value.msgs) || 0), 0), estimatedPrice: entries.reduce((sum, [, value]) => sum + (Number(value.estimatedPrice) || 0), 0), unknownPrice: entries.reduce((sum, [, value]) => sum + (Number(value.unknownPrice) || 0), 0) }))}</b><span class="m-tok">${fmt(totTok)}</span></div></div>`;
  bm.innerHTML = html;
}
...
function renderChart(hourlyCost, hourlyTokens) {
  ...
  el.innerHTML = values
    ...
    .join('');
  hoursSummary = usageMetric === 'cost' ? t('panel.hoursSummaryCost', { total: total.toFixed(2), peakH, peakV: peakV.toFixed(2) }) : t('panel.hoursSummaryTokens', { total: fmt(total), peakH, peakV: fmt(peakV) });
...
function renderCal(daily) {
  ...
  el.innerHTML = html;
  calSummary = usageMetric === 'cost' ? t('panel.calSummaryCost', { n: list.length, total: fmtCost(total) }) : t('panel.calSummaryTokens', { n: list.length, total: fmt(total) });
...
function renderSessList(sessions) {
  ...
  el.innerHTML = filtered
    .map((s) => {
      ...
      return `<div class="row sess${isPinned ? ' pinned' : ''}${isArchived ? ' archived' : ''}" data-provider="${escapeHtml(provider)}" title="${escapeHtml(sid)}"><span class="badge ${m.cls}">${escapeHtml(t(m.key))}</span><span class="sess-provider" title="${escapeHtml(meta.label)}">${escapeHtml(meta.icon)}</span><span class="sess-proj">${escapeHtml(s.project)}</span><span class="sess-id">${escapeHtml(sidShort)}</span><span class="sess-op">${detail}</span><span class="sess-actions">${pinBtn}${archiveBtn}</span></div>`;
    })
    .join('');
  ...
  el.querySelectorAll('.sess-pin, .sess-archive').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      ...
      const enabled = action === 'pin' ? sessionPinned.includes(sid) : sessionArchived.includes(sid);
      const request = Promise.resolve().then(() => window.OctoSessionPrefs.save(window.pet, sid, action, enabled, sessionPinned, sessionArchived));
      request.catch((err) => {
          ...
          window.dispatchEvent(new CustomEvent('re-llmpet:bridge-error', { detail: { command: window.pet.setSessionPref ? 'set_session_pref' : 'set_session_prefs', message: msg } }));
        })
...
function renderProviderDiagnostic(result) {
  ...
  const projectOverlays = configInfo && Array.isArray(configInfo.projectOverlays)
    ...
    : '';
  const configRows = configInfo ? [diagnosticRow(t('diag.config'), configInfo.selected || configInfo.path, true), diagnosticRow(t('diag.configSource'), configInfo.selectedSource, false), diagnosticRow(t('diag.projectConfig'), projectOverlays, true), diagnosticRow(t('diag.hooks'), configInfo.reLlmpetHookBlock ? t('diag.present') : t('diag.absent'), false), diagnosticRow(t('diag.legacyModels'), legacyModels, true)].join('') : '';
  ...
  const aiderCredentials = aiderSummary && Array.isArray(aiderSummary.credentialEnvironment)
    ...
    : '';
  const aiderRows = aiderSummary ? [diagnosticRow(t('diag.configCandidates'), aiderConfigs || t('diag.absent'), true), diagnosticRow(t('diag.credentialHints'), aiderCredentials || t('diag.absent'), true), diagnosticRow(t('diag.modelEnvironment'), aiderSummary.modelEnvironment ? t('diag.present') : t('diag.absent'), false)].join('') : '';
  const terminalRows = terminal ? [diagnosticRow('Windows Terminal', terminal.windowsTerminal || t('diag.absent'), true), diagnosticRow('cmd.exe', terminal.cmdFallback || t('diag.absent'), true)].join('') : '';
  const doctorRows = doctorSummary ? [diagnosticRow(t('diag.doctorStatus'), doctorSummary.status, false), diagnosticRow(t('diag.providerRoute'), doctorSummary.provider, false), diagnosticRow(t('diag.modelRoute'), doctorSummary.model, true), diagnosticRow(t('diag.apiKeySource'), doctorSummary.apiKeySource, false), diagnosticRow(t('diag.payloadMode'), doctorSummary.requestPayloadMode, false), diagnosticRow(t('diag.sessionRecovery'), doctorSummary.sessionRecovery, false)].join('') : '';
  el.innerHTML = `
    <div class="diag-head">
...
function renderProviders() {
  ...
  el.innerHTML = all.map((id) => {
    ...
    return '<div class="prov-item' + (on ? ' active' : '') + (locked ? ' locked' : '') + '"><label class="prov-toggle"><input type="checkbox" ' + (on ? 'checked' : '') + ' ' + (locked ? 'disabled' : '') + ' data-id="' + escapeHtml(id) + '"><span class="prov-icon">' + escapeHtml(m.icon) + '</span><span class="prov-label">' + escapeHtml(m.label) + '</span>' + status + '</label><button type="button" class="prov-diagnose" data-provider="' + escapeHtml(id) + '" ' + (busy ? 'disabled' : '') + '>' + escapeHtml(busy ? t('diag.runningShort') : t('diag.action')) + '</button></div>';
  }).join('');
}
