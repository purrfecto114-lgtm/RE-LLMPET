...
const checks = [
  ...
  ['Windows native and shim launch paths both receive provider arguments', commands.includes('cmd_launch_call(executable, &launch_args)') && commands.includes('command.arg(executable).args(launch_args.iter().cloned())')],
  ...
  ['macOS and Linux launch paths retain provider arguments', commands.includes('launch_args.extend(extra_args.iter().cloned())') && commands.includes('command.extend(launch_args.iter().map(|arg| shell_quote(arg)))') && commands.includes('provider_args.iter().cloned()')],
