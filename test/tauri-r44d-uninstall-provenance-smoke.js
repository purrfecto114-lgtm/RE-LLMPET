...
// R44 Phase 0D (2026-08-03) — 0.5.57 uninstall provenance + drift detection.
//
...
//          0.5.57), uninstall still succeeds; priorReceipt is null
...
assert.strictEqual(packageJson.version, '0.5.57',
...
// R44 0.5.57: the single-provider and bulk paths now share a `run_one`
...
// ──────────────────────────────────────────────────────────────────────────
// P0D-5: "all" uninstall path reuses single-provider pipeline (0.5.57 §4)
// ──────────────────────────────────────────────────────────────────────────

// R44 0.5.57 (roadmap v5 §4): "all" must call the same run_one pipeline
...
assert.ok(commands.includes('"allHooksVerifiedAbsent": all_clean'),
  'P0D-5: "all" uninstall must return allHooksVerifiedAbsent (0.5.57 §4)');
...
assert.ok(commands.includes('all_providers.to_vec()'),
  'P0D-5: "all" must iterate all_providers via the same run_one pipeline (0.5.57 §4)');
...
console.log('✓ R44 Phase 0D (0.5.57) uninstall provenance smoke: all assertions passed');
