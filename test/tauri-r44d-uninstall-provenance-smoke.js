...
// R44 Phase 0D (2026-08-03) — 0.5.56 uninstall provenance + drift detection.
//
...
//          0.5.56), uninstall still succeeds; priorReceipt is null
...
// ──────────────────────────────────────────────────────────────────────────
// Version (still 0.5.56 — Phase 0D ships in the same release as 0C)
// ──────────────────────────────────────────────────────────────────────────

assert.strictEqual(packageJson.version, '0.5.56',
  'P0D: package.json version must remain 0.5.56 (Phase 0C+0D ship together)');
...
// R44 0.5.56: the single-provider and bulk paths now share a `run_one`
...
// ──────────────────────────────────────────────────────────────────────────
// P0D-5: "all" uninstall path reuses single-provider pipeline (0.5.56 §4)
// ──────────────────────────────────────────────────────────────────────────

// R44 0.5.56 (roadmap v5 §4): "all" must call the same run_one pipeline
...
assert.ok(commands.includes('"allHooksVerifiedAbsent": all_clean'),
  'P0D-5: "all" uninstall must return allHooksVerifiedAbsent (0.5.56 §4)');
...
assert.ok(commands.includes('all_providers.to_vec()'),
  'P0D-5: "all" must iterate all_providers via the same run_one pipeline (0.5.56 §4)');
...
console.log('✓ R44 Phase 0D (0.5.56) uninstall provenance smoke: all assertions passed');
