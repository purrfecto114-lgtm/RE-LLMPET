...
// R44 Phase 0C (2026-08-03) — 0.5.50 unified backup + install receipt smoke.
//
...
assert.strictEqual(packageJson.version, '0.5.50',
  'P0C: package.json version must be 0.5.50');
assert.ok(cargoToml.includes('version = "0.5.50"'),
  'P0C: Cargo.toml version must be 0.5.50');
assert.strictEqual(tauriConf.version, '0.5.50',
  'P0C: tauri.conf.json version must be 0.5.50');
assert.ok(changelog.includes('0.5.50'),
  'P0C: CHANGELOG must have 0.5.50 entry');
...
console.log('✓ R44 Phase 0C (0.5.50) backup + receipt smoke: all assertions passed');
