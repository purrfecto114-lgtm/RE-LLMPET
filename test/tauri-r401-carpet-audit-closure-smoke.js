...
// R40.1 (2026-08-01) — 0.5.34 carpet audit closure smoke.
//
...
assert.strictEqual(packageJson.version, '0.5.34',
...
assert(changelog.includes('0.5.34'),
  'P0-5: CHANGELOG must have 0.5.34 entry');
assert(changelog.includes('0.5.34'),
...
//   - a human-readable identifier like 're-llmpet-0.5.34' (for local dev)
...
assert.strictEqual(manifest.version, '0.5.34',
  'P0-5: manifest version must be 0.5.34');
...
assert.strictEqual(manifest.root, `RE-LLMPET-0.5.34`,
  `P0-5: manifest.root must be RE-LLMPET-0.5.34 (got ${manifest.root})`);
