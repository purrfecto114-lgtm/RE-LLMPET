...
// R40.1 (2026-08-01) — 0.5.30 carpet audit closure smoke.
//
...
assert.strictEqual(packageJson.version, '0.5.30',
...
assert(changelog.includes('0.5.30'),
  'P0-5: CHANGELOG must have 0.5.30 entry');
assert(changelog.includes('0.5.30'),
...
//   - a human-readable identifier like 're-llmpet-0.5.30' (for local dev)
...
assert.strictEqual(manifest.version, '0.5.30',
  'P0-5: manifest version must be 0.5.30');
...
assert.strictEqual(manifest.root, `RE-LLMPET-0.5.30`,
  `P0-5: manifest.root must be RE-LLMPET-0.5.30 (got ${manifest.root})`);
