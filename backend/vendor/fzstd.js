"use strict";
// ─────────────────────────────────────────────────────────────────────────────
// VENDORED THIRD-PARTY CODE — DO NOT EDIT BY HAND.
//
//   fzstd 0.1.1 (CommonJS build, node_modules/fzstd/lib/index.js)
//   https://github.com/101arrowz/fzstd — MIT, (c) 2020 Arjun Barrett
//   完整许可证见同目录 LICENSE-fzstd。
//
// 为什么内置而不是走 npm 依赖：
//   1. DeepSeek Harness 的会话日志默认是 zstd 分帧压缩，而 Electron 33 带的
//      Node 20.18 的 zlib 没有 zstd API（实测 zlib 里一个 zstd* 符号都没有），
//      dsh-watch 没有解码器就什么都读不到；
//   2. scripts/package-mac.sh 是「显式清单拷贝」，根本不拷 node_modules，
//      加 dependencies 会打出一个跑不起来的 mac 包；
//   3. 仓库此前零运行时依赖，一个 24KB 无依赖的纯 JS 解码器内置进来最省事。
// 升级方式：npm pack fzstd@<ver> 解包后覆盖本文件正文（保留这段抬头），跑 npm test。
// ─────────────────────────────────────────────────────────────────────────────
