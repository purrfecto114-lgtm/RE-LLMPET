...
const root=path.resolve(__dirname,'..'); const out=path.resolve(process.argv[2]||'reports/re-llmpet.spdx.json');
const pkg=JSON.parse(fs.readFileSync(path.join(root,'package.json')));
const packages=[{SPDXID:'SPDXRef-Package-RE-LLMPET',name:pkg.name,versionInfo:pkg.version,downloadLocation:'NOASSERTION',filesAnalyzed:false,licenseConcluded:pkg.license||'NOASSERTION',licenseDeclared:pkg.license||'NOASSERTION',copyrightText:'NOASSERTION'}];
...
const relationships=[{spdxElementId:'SPDXRef-DOCUMENT',relationshipType:'DESCRIBES',relatedSpdxElement:'SPDXRef-Package-RE-LLMPET'},...packages.slice(1).map(p=>({spdxElementId:'SPDXRef-Package-RE-LLMPET',relationshipType:'DEPENDS_ON',relatedSpdxElement:p.SPDXID}))];
const doc={spdxVersion:'SPDX-2.3',dataLicense:'CC0-1.0',SPDXID:'SPDXRef-DOCUMENT',name:`RE-LLMPET-${pkg.version}`,documentNamespace:ns,creationInfo:{created:new Date().toISOString(),creators:['Tool: scripts/generate-sbom.js']},packages,relationships};
