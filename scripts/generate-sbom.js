...
const ns=`https://github.com/octopus/llmpet/spdx/${pkg.version}/${crypto.randomUUID()}`;
const doc={spdxVersion:'SPDX-2.3',dataLicense:'CC0-1.0',SPDXID:'SPDXRef-DOCUMENT',name:`Octopus-${pkg.version}`,documentNamespace:ns,creationInfo:{created:new Date().toISOString(),creators:['Tool: scripts/generate-sbom.js']},packages,relationships:packages.slice(1).map(p=>({spdxElementId:'SPDXRef-Package-Octopus',relationshipType:'DEPENDS_ON',relatedSpdxElement:p.SPDXID}))};
