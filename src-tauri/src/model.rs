...
pub struct AppConfig {
    ...
    /// Custom provider launch specs — user-defined providers not in the built-in registry.
    /// Each spec defines a safe launch configuration (no shell, validated args).
    #[serde(default)]
...
impl AppConfig {
    pub fn sanitize(mut self) -> Self {
        ...
        // Validate built-in + custom provider IDs
        let custom_ids: Vec<String> = self.custom_providers.iter().map(|c| c.id.clone()).collect();
        self.providers = validate_provider_ids(self.providers, &custom_ids);
        // Validate custom provider specs
        let mut custom_specs = self.custom_providers.clone();
        let _errors = validate_custom_provider_specs(&mut custom_specs);
        self.custom_providers = custom_specs;
        ...
        // self.providers = providers; // Already validated above
    ...
    pub fn ingest(&self, body: &Value) -> Session {
        ...
        let (snapshot, accepted) = {
            ...
            if accepted {
                ...
                // Child-session identity is learned from session.created and
