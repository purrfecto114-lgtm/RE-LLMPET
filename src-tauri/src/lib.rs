            ...
            #[cfg(target_os = "windows")]
            {
                for label in ["pet", "pet-codex", "panel"] {
                    if let Some(window) = app.get_webview_window(label) {
                        ...
                        // .ico so Windows picks the correct size for each DPI scale.
...
fn build_tray_menu<R: tauri::Runtime>(
    ...
    // New-agent submenu: only show launch items for enabled providers.
    // Item ids stay stable across languages so the on_menu_event handler
    // never needs to know which language produced the label.
    // P5-4 fix: filter by config.providers so disabled agents are hidden.
    ...
    let mut launch_menu_builder =
        SubmenuBuilder::new(app, i18n::tray_label(lang, "tray.launchAgent"));
    if providers.iter().any(|p| p == "claude") {
        launch_menu_builder = launch_menu_builder.item(&launch_claude);
    }
    if providers.iter().any(|p| p == "codewhale") {
        launch_menu_builder = launch_menu_builder.item(&launch_cw);
    }
    if providers.iter().any(|p| p == "codex") {
        launch_menu_builder = launch_menu_builder.item(&launch_codex);
    }
    if providers.iter().any(|p| p == "opencode") {
        launch_menu_builder = launch_menu_builder.item(&launch_opencode);
    }
    if providers.iter().any(|p| p == "aider") {
        launch_menu_builder = launch_menu_builder.item(&launch_aider);
    }
    let launch_menu = launch_menu_builder.build()?;
...
pub fn refresh_tray_menu(app: &tauri::AppHandle) {
    ...
    match build_tray_menu(app, &lang, &config.providers) {
        Ok(menu) => {
...
fn setup_tray(app: &mut tauri::App) -> tauri::Result<()> {
    ...
    let config = app.state::<AppState>().runtime.config();
    let lang = config.lang;
    let menu = build_tray_menu(app, &lang, &config.providers)?;
