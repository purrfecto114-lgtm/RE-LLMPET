...
pub fn get_config_state(state: State<'_, AppState>) -> Value {
    ...
    let (label, message) = match &cs {
        ...
        crate::model::ConfigState::TooLarge { size } => ("tooLarge", Some(format!("file is {} bytes", size))),
        crate::model::ConfigState::SchemaTooNew { version } => ("schemaTooNew", Some(format!("schema version {} is newer than this build supports", version))),
    };
    json!({
        "state": label,
...
pub fn backup_and_reset_config(
    ...
    let backup_name = result.backup_path.as_ref().and_then(|p| p.file_name()).map(|n| n.to_string_lossy().into_owned()).unwrap_or_else(|| "<none>".to_string());
    state.runtime.write_log("config", &format!("backup_and_reset_config: reset={}, backup_created={}, backup_name={}", result.reset, result.backup_created, backup_name));
...
pub fn start_travel(
    ...
    state.runtime.travel.start_project(app, state.runtime.clone(), session_id, mission.unwrap_or_else(|| pick_travel_mission().into()))
}
...
pub fn start_wander(
    ...
    state.runtime.travel.start_wander(app, state.runtime.clone(), mission.unwrap_or_else(|| pick_wander_mission().into()), provider)
}
...
pub fn rebuild_usage_costs(app: AppHandle, state: State<'_, AppState>) -> Result<Value, String> {
    let mut usage = state.runtime.usage.lock().unwrap_or_else(|error| error.into_inner());
    ...
    drop(usage);
    state.runtime.write_log("metering", &format!("rebuild_costs: {count} events, ${before:.4} → ${after:.4}"));
...
pub fn uninstall_hooks(
    ...
    let run_one = |id: &str| -> (Value, bool, Option<String>) {
        ...
        let prior_receipt = crate::hook_install::read_install_receipts().get(id).cloned();
        ...
        let receipt_path = prior_receipt.as_ref().and_then(|r| r.get("path")).and_then(Value::as_str).map(Path::new);
        ...
        let failure_msg = if cleanup.is_hard_failure() { Some(format!("{}: {:?}", id, cleanup)) } else { None };
    ...
    if provider == "all" {
        let message = if all_clean {
            ...
            format!("Provider selection cleared. Some external hooks could not be fully verified: {}", failures.join("；"))
        };
        Ok(json!({
            "provider": "all",
...
pub fn set_pet_mode(
    ...
    let config = state.runtime.update_config(|config| config.pet_mode = pet_mode.clone())?;
    sync_pet_windows(&app, &config);
    emit_config(&app, &state);
...
pub fn set_session_prefs(
    ...
    fn sanitize_ids(ids: Vec<String>) -> Vec<String> {
        ...
        ids.into_iter().map(|s| s.trim().chars().take(256).collect::<String>()).filter(|s| !s.is_empty() && seen.insert(s.clone())).collect()
    }
    let pinned_clean = sanitize_ids(pinned);
    let pinned_set: std::collections::HashSet<&str> = pinned_clean.iter().map(String::as_str).collect();
    let archived_clean = sanitize_ids(archived).into_iter().filter(|id| !pinned_set.contains(id.as_str())).collect::<Vec<_>>();
    state.runtime.update_config(|config| {
    ...
    state.runtime.update_config(|config| {
        if let Some(enabled) = pinned {
            ...
            if enabled {
                ...
                config.archived_sessions.retain(|value| value != &session_id);
            }
        }
...
pub fn set_providers(
    ...
    state.runtime.update_config(|config| config.providers = ids.clone())?;
    ...
    let providers: Vec<Value> = statuses
        ...
        .collect();
    let errors: Vec<String> = statuses.iter().filter(|s| s.state == "error").map(|s| format!("{}: {}", s.id, s.message)).collect();
...
pub fn territory_toggle_auto(
    ...
    if !config.territory {
        ...
        let _ = app.emit("pet:event", json!({"kind":"say","text":"Territory mode disabled."}));
        return Ok(result);
    }
...
fn fit_panel(
    ...
    let pet_monitor = app.get_webview_window("pet").and_then(|pet| pet.current_monitor().ok().flatten());
    ...
    }.or(window.primary_monitor().map_err(|error| error.to_string())?).ok_or("monitor information unavailable")?;
    ...
    let current_height = window.outer_size().map(|size| f64::from(size.height) / scale).unwrap_or(PANEL_DEFAULT_HEIGHT);
    let desired_height = requested_height.filter(|value| value.is_finite() && *value > 0.0).unwrap_or(current_height);

    let logical_width = PANEL_DEFAULT_WIDTH.min(max_width).max(PANEL_MIN_WIDTH.min(max_width));
    let logical_height = desired_height.min(PANEL_DEFAULT_HEIGHT).min(max_height).max(PANEL_MIN_HEIGHT.min(max_height));
    let physical_width = logical_to_physical(logical_width, scale);
    let physical_height = logical_to_physical(logical_height, scale);

    let centered_x = i64::from(work.position.x) + (i64::from(work.size.width) - i64::from(physical_width)) / 2;
    let centered_y = i64::from(work.position.y) + (i64::from(work.size.height) - i64::from(physical_height)) / 2;
    let (candidate_x, candidate_y) = match placement {
        PanelPlacement::CenterOnPet => (centered_x, centered_y),
        PanelPlacement::PreserveCurrentCenter => match (window.outer_position(), window.outer_size()) {
            (Ok(position), Ok(size)) => (
                i64::from(position.x) + (i64::from(size.width) - i64::from(physical_width)) / 2,
                i64::from(position.y) + (i64::from(size.height) - i64::from(physical_height)) / 2,
            ),
    ...
    let max_x = (i64::from(work.position.x) + i64::from(work.size.width) - margin - i64::from(physical_width)).max(min_x);
    let max_y = (i64::from(work.position.y) + i64::from(work.size.height) - margin - i64::from(physical_height)).max(min_y);
    let x = candidate_x.clamp(min_x, max_x);
    let y = candidate_y.clamp(min_y, max_y);

    window.set_size(Size::Physical(PhysicalSize::new(physical_width, physical_height))).map_err(|error| error.to_string())?;
...
pub fn set_win_pos(app: AppHandle, x: i32, y: i32, agent: Option<String>) -> Result<(), String> {
    ...
    window.set_position(Position::Physical(PhysicalPosition::new(physical_x, physical_y))).map_err(|error| error.to_string())
}
...
pub fn commit_win_pos(
    ...
    let logical_y = (position.y as f64 / scale).round() as i32;
    state.runtime.update_config(|config| {
        let point = Some(Point { x: logical_x, y: logical_y });
        if agent.as_deref() == Some("codex") {
...
pub fn set_pet_tall(app: AppHandle, tall: bool, agent: Option<String>) -> Result<(), String> {
    set_pet_size(app, 320.0, if tall { 620.0 } else { 340.0 }, agent).map(|_| ())
}

#[tauri::command]
pub fn set_pet_big(app: AppHandle, on: bool, agent: Option<String>) -> Result<(), String> {
    set_pet_size(app, if on { 520.0 } else { 320.0 }, if on { 700.0 } else { 340.0 }, agent).map(|_| ())
}
...
pub struct PetResizeLayout {
    anchor_shift_x: f64,
    anchor_shift_y: f64,
    side: &'static str,
}

/// Keep the visual anchor stable while strictly bounding the native window to
/// its monitor work area. The returned shift is applied by the renderer so the
/// HUD grows into available space rather than moving the pet.
fn place_pet_anchor(geometry: PetAnchorGeometry) -> PetPlacement {
    let mut width = geometry.target_width.max(1);
    let mut height = geometry.target_height.max(1);
    let mut anchor_x = geometry.old_x + i64::from(geometry.old_width) / 2;
    let mut anchor_y = geometry.old_y + i64::from(geometry.old_height);

    if let Some(work) = geometry.work_area {
        width = width.min(work.width.max(1));
        height = height.min(work.height.max(1));
        // Only recover an already-invalid anchor. A valid anchor must not move
        // merely because its surrounding HUD becomes larger.
        anchor_x = anchor_x.clamp(work.x, work.x + i64::from(work.width));
        anchor_y = anchor_y.clamp(work.y, work.y + i64::from(work.height));
    }

    let (x, y) = if let Some(work) = geometry.work_area {
        let max_x = work.x + i64::from(work.width) - i64::from(width);
        let max_y = work.y + i64::from(work.height) - i64::from(height);
        (
            (anchor_x - i64::from(width) / 2).clamp(work.x, max_x),
            (anchor_y - i64::from(height)).clamp(work.y, max_y),
        )
    } else {
        (anchor_x - i64::from(width) / 2, anchor_y - i64::from(height))
    };
    let anchor_shift_x = anchor_x - (x + i64::from(width) / 2);
    let anchor_shift_y = anchor_y - (y + i64::from(height));
    let side = if anchor_shift_x > 0 { "right" }
        else if anchor_shift_x < 0 { "left" }
        else if anchor_shift_y > 0 { "bottom" }
        else if anchor_shift_y < 0 { "top" }
        else { "center" };
    PetPlacement { x, y, width, height, anchor_shift_x, anchor_shift_y, side }
}
...
fn resize_pet_anchored(
    ...
    //
    let scale = window.scale_factor().ok().filter(|value| value.is_finite() && *value > 0.0).unwrap_or(1.0);
    ...
    let work_area = window.current_monitor().map_err(|error| error.to_string())?
        .map(|monitor| {
            let work = monitor.work_area();
            PetWorkArea {
    ...
    let placement = place_pet_anchor(PetAnchorGeometry {
        old_x: i64::from(old_position.x),
        old_y: i64::from(old_position.y),
        old_width: old_size.width,
        old_height: old_size.height,
        target_width: logical_to_physical(width, scale),
        target_height: logical_to_physical(height, scale),
        work_area,
    });

    window.set_size(Size::Physical(PhysicalSize::new(placement.width, placement.height))).map_err(|error| error.to_string())?;
    window.set_position(Position::Physical(PhysicalPosition::new(
        placement.x.clamp(i64::from(i32::MIN), i64::from(i32::MAX)) as i32,
        placement.y.clamp(i64::from(i32::MIN), i64::from(i32::MAX)) as i32,
    ))).map_err(|error| error.to_string())?;
    Ok(PetResizeLayout {
        anchor_shift_x: placement.anchor_shift_x as f64 / scale,
        anchor_shift_y: placement.anchor_shift_y as f64 / scale,
        side: placement.side,
    })
}
...
pub fn set_panel_height(app: AppHandle, height: f64) -> Result<[f64; 2], String> {
    let window = app.get_webview_window("panel").ok_or("panel window missing")?;
    fit_panel(&app, &window, Some(height), PanelPlacement::PreserveCurrentCenter)
}
    ...
    match value {
        Value::Object(object) => {
            for (key, child) in object.iter_mut() {
                ...
                let sensitive = matches!(normalized.as_str(), "authorization" | "access_token" | "refresh_token" | "client_secret" | "password" | "token" | "secret" | "api_key_value");
                if sensitive {
                ...
                if normalized == "api_key" {
                    let source = child.get("source").and_then(Value::as_str).map(str::to_owned);
                    *child = source.map(|source| json!({"source": source})).unwrap_or_else(|| Value::String("***".into()));
                    continue;
                }
...
fn bounded_probe_text(bytes: &[u8]) -> String {
    let filtered: String = decode_subprocess_output(bytes).chars().filter(|ch| !ch.is_control() || *ch == '\n' || *ch == '\t').take(16_384).collect();
    filtered.lines().map(redact_sensitive_line).collect::<Vec<_>>().join("\n").chars().take(8192).collect()
}
...
fn decode_subprocess_output(bytes: &[u8]) -> String {
    ...
    if bytes.starts_with(&[0xFF, 0xFE]) {
        let utf16: Vec<u16> = bytes[2..].chunks_exact(2).map(|pair| u16::from_le_bytes([pair[0], pair[1]])).collect();
        if let Ok(text) = String::from_utf16(&utf16) {
            return text;
        }
    }
    if bytes.starts_with(&[0xFE, 0xFF]) {
        let utf16: Vec<u16> = bytes[2..].chunks_exact(2).map(|pair| u16::from_be_bytes([pair[0], pair[1]])).collect();
        if let Ok(text) = String::from_utf16(&utf16) {
            return text;
        }
    }
...
fn decode_windows_codepage(bytes: &[u8]) -> Option<String> {
    ...
    fn try_codepage(codepage: u32, bytes: &[u8]) -> Option<String> {
        ...
        let needed = unsafe { MultiByteToWideChar(codepage, flags, bytes.as_ptr(), bytes.len() as i32, core::ptr::null_mut(), 0) };
        if needed <= 0 {
            return None;
        }
        let mut buffer = vec![0u16; needed as usize];
        let written = unsafe { MultiByteToWideChar(codepage, flags, bytes.as_ptr(), bytes.len() as i32, buffer.as_mut_ptr(), needed) };
        if written <= 0 {
            return None;
        }
...
fn run_diagnostic_probe_capture(
    ...
    let mut child = match command.current_dir(cwd).stdin(Stdio::null()).stdout(Stdio::piped()).stderr(Stdio::piped()).spawn() {
        Ok(child) => child,
        Err(error) => {
            return ProbeCapture {
    ...
    let stdout_reader = thread::spawn(move || stdout.map(|pipe| drain_bounded(pipe, 64 * 1024).unwrap_or_default()).unwrap_or_default());
    let stderr_reader = thread::spawn(move || stderr.map(|pipe| drain_bounded(pipe, 64 * 1024).unwrap_or_default()).unwrap_or_default());

    if !control.register_pid(pid) {
        let _ = kill_process_tree(pid);
        let _ = child.wait();
        let _ = stdout_reader.join().map_err(|e| eprintln!("stdout reader thread panicked: {e:?}"));
        let _ = stderr_reader.join().map_err(|e| eprintln!("stderr reader thread panicked: {e:?}"));
        return cancelled_probe_capture();
    }
            ...
            Err(error) => {
                if control.claim_pid_for_termination(pid) {
                    if let Err(kill_error) = kill_process_tree(pid) {
                        termination_error = Some(format!("failed to poll child process: {error}; termination failed: {kill_error}"));
                    } else {
    ...
    let parsed_json = if cancelled || timed_out { None } else { sanitized_probe_json(&stdout).or_else(|| sanitized_probe_json(&stderr)) };
    ProbeCapture {
        report: json!({
            "started": true,
            "timedOut": timed_out,
...
fn probe_succeeded(probe: &Value) -> bool {
    probe.get("started").and_then(Value::as_bool).unwrap_or(false)
        && !probe.get("timedOut").and_then(Value::as_bool).unwrap_or(false)
        && probe.get("success").and_then(Value::as_bool).unwrap_or(false)
}

fn probe_failure_detail(probe: &Value) -> String {
    if probe.get("timedOut").and_then(Value::as_bool).unwrap_or(false) {
...
fn probe_combined_text(probe: &Value) -> String {
    ["stdout", "stderr", "error"].into_iter().filter_map(|key| probe.get(key).and_then(Value::as_str)).collect::<Vec<_>>().join("\n")
}

fn probe_indicates_unknown_command(probe: &Value) -> bool {
    let text = probe_combined_text(probe).to_ascii_lowercase();
    ["unknown command", "unknown subcommand", "unrecognized subcommand", "invalid subcommand", "unexpected argument 'doctor'", "unexpected argument \"doctor\"", "found argument 'doctor' which wasn't expected", "found argument \"doctor\" which wasn't expected"].iter().any(|needle| text.contains(needle))
}
...
fn codewhale_doctor_probe(
    ...
    let mut companion_capture = companion.map(|path| {
        let capture = run_diagnostic_probe_capture(path, &["doctor", "--json"], cwd, Duration::from_secs(8), control);
        attempts.push(json!({
            "surface": "companion",
    ...
    let should_try_dispatcher = !companion_is_definitive
        && match companion_capture.as_ref() {
            None => true,
            Some((_, capture)) => probe_indicates_unknown_command(&capture.report) || !capture.report.get("started").and_then(Value::as_bool).unwrap_or(false),
        };
    ...
    if should_try_dispatcher {
        if let Some(path) = dispatcher {
            ...
            if !duplicate {
                ...
                let dispatcher_is_better = capture.json.is_some() || probe_succeeded(&capture.report) || companion_capture.as_ref().is_some_and(|(_, companion)| probe_indicates_unknown_command(&companion.report));
...
fn numeric_version_parts(value: &str) -> Vec<u64> {
    value.split(['.', '-', '+']).take_while(|part| part.chars().all(|ch| ch.is_ascii_digit())).map(|part| part.parse::<u64>().unwrap_or(0)).collect()
}
...
fn semverish_from_probe(probe: &Value) -> Option<String> {
    ["stdout", "stderr"].into_iter().filter_map(|key| probe.get(key).and_then(Value::as_str)).flat_map(str::split_whitespace).map(|value| value.trim_matches(|ch: char| !(ch.is_ascii_alphanumeric() || ch == '.' || ch == '-' || ch == '+')).trim_start_matches('v').to_string()).find(|value| value.chars().next().is_some_and(|ch| ch.is_ascii_digit()) && value.matches('.').count() >= 1)
}
...
fn codewhale_doctor_summary(value: Option<&Value>) -> Value {
    ...
    let session_recovery = report.get("legacy_state").and_then(|legacy| legacy.get("session_recovery")).and_then(|recovery| recovery.get("status").and_then(Value::as_str).or_else(|| recovery.as_str()));
    json!({
...
fn codewhale_config_candidates(cwd: &Path) -> Value {
    ...
    let explicit = std::env::var_os("CODEWHALE_CONFIG_PATH").map(|path| ("CODEWHALE_CONFIG_PATH", PathBuf::from(path)))
        .or_else(|| std::env::var_os("DEEPSEEK_CONFIG_PATH").map(|path| ("DEEPSEEK_CONFIG_PATH", PathBuf::from(path))))
        .or_else(|| std::env::var_os("CODEWHALE_HOME").map(|base| ("CODEWHALE_HOME", PathBuf::from(base).join("config.toml"))));
...
fn codewhale_config_compatibility(path: &Path) -> Value {
    ...
    let active_lines = raw.lines().map(|line| line.split('#').next().unwrap_or_default().trim().to_ascii_lowercase()).filter(|line| !line.is_empty()).collect::<Vec<_>>();
    let active_text = active_lines.join("\n");
    let legacy_model_ids = ["deepseek-chat", "deepseek-reasoner"].into_iter().filter(|model| active_text.contains(model)).collect::<Vec<_>>();
...
fn aider_configuration_summary(cwd: &Path) -> Value {
    ...
    let credential_environment = ["OPENAI_API_KEY", "ANTHROPIC_API_KEY", "DEEPSEEK_API_KEY", "OPENROUTER_API_KEY", "GEMINI_API_KEY", "AZURE_API_KEY"].into_iter().filter(|name| std::env::var_os(name).is_some_and(|value| !value.to_string_lossy().trim().is_empty())).collect::<Vec<_>>();
    let model_environment = std::env::var_os("AIDER_MODEL").is_some_and(|value| !value.to_string_lossy().trim().is_empty());
    json!({
...
fn executable_kind(path: &Path) -> &'static str {
    #[cfg(windows)]
    {
        ...
        if path.extension().and_then(|value| value.to_str()).is_some_and(|value| value.eq_ignore_ascii_case("exe") || value.eq_ignore_ascii_case("com")) {
...
pub async fn diagnose_agent(
    ...
    let task = tauri::async_runtime::spawn_blocking(move || {
        let _ = app_for_worker.emit("panel:diagnostic-progress", json!({"provider": provider_for_worker, "phase": "starting"}));
...
pub async fn cancel_diagnostic(state: State<'_, AppState>) -> Result<Value, String> {
    ...
    for pid in &request.pids {
        match kill_process_tree(*pid) {
            Ok(()) => killed.push(*pid),
            Err(error) => {
                state.runtime.diagnostic_control.restore_pid_after_failed_termination(*pid);
...
pub(crate) fn kill_process_tree(pid: u32) -> Result<(), String> {
    #[cfg(windows)]
    {
        ...
        command.args(["/F", "/T", "/PID", &pid.to_string()]).stdin(Stdio::null()).stdout(Stdio::piped()).stderr(Stdio::piped());
        // R22: hide the flashing taskkill.exe console window on probe cancel.
        crate::platform::hide_console_window(&mut command);
        let output = command.output().map_err(|e| format!("failed to spawn taskkill: {e}"))?;
...
fn diagnose_agent_sync(provider: String, control: &DiagnosticControl) -> Result<Value, String> {
    ...
    let companion = executable.as_deref().and_then(|path| companion_for(spec, path));
    ...
    let scope_result = thread::scope(|s| {
        ...
        let version_handle = s.spawn(move || exe_v.as_deref().map_or(Value::Null, |path| run_diagnostic_probe(path, &["--version"], &wd_v, Duration::from_secs(5), control)));
        ...
        let companion_version_handle = s.spawn(move || {
            if spec_id == "codewhale" {
                comp_cv.as_deref().map_or(Value::Null, |path| run_diagnostic_probe(path, &["--version"], &wd_cv, Duration::from_secs(5), control))
            } else {
                Value::Null
            }
        });
        ...
        let doctor_handle = s.spawn(move || match spec_id {
            ...
            "claude" => (
                exe_d.as_deref().map_or(Value::Null, |path| run_diagnostic_probe(path, &["doctor"], &wd_d, Duration::from_secs(8), control)),
        ...
        let auth_handle = s.spawn(move || match spec_id {
            "codewhale" => exe_a.as_deref().map_or(Value::Null, |path| run_diagnostic_probe(path, &["auth", "status"], &wd_a, Duration::from_secs(8), control)),
            "codex" => exe_a.as_deref().map_or(Value::Null, |path| run_diagnostic_probe(path, &["login", "status"], &wd_a, Duration::from_secs(8), control)),
        ...
        (
            version_handle.join().unwrap_or_else(|_| {
                eprintln!("version probe thread panicked");
                Value::Null
            }),
            companion_version_handle.join().unwrap_or_else(|_| {
                eprintln!("companion version probe thread panicked");
                Value::Null
            }),
            doctor_handle.join().unwrap_or_else(|_| {
                eprintln!("doctor probe thread panicked");
                (Value::Null, None, None, None, Value::Null)
            }),
            auth_handle.join().unwrap_or_else(|_| {
                eprintln!("auth probe thread panicked");
                Value::Null
            }),
        )
    });
    ...
    if executable.is_some() && !probe_succeeded(&version) {
        issues.push(format!("{} --version failed: {}", spec.title, probe_failure_detail(&version)));
    }
    if spec.id == "claude" {
        if let Some(actual) = semverish_from_probe(&version) {
            if version_is_older(&actual, "2.1.200") {
                warnings.push(format!("Claude Code {actual} predates the 2.1.200 sleep/wake and background-session reliability fixes; upgrade before attributing resumed-session authentication failures to Octopus"));
            }
        }
    }
    if spec.id == "codewhale" && companion.is_some() && !probe_succeeded(&companion_version) {
        issues.push(format!("codewhale-tui --version failed: {}", probe_failure_detail(&companion_version)));
    }
    if spec.id == "codewhale" && probe_succeeded(&version) && probe_succeeded(&companion_version) {
        ...
        if dispatcher_version.is_some() && runtime_version.is_some() && dispatcher_version != runtime_version {
            issues.push(format!("CodeWhale dispatcher/runtime version mismatch: {} vs {}", dispatcher_version.unwrap_or_default(), runtime_version.unwrap_or_default()));
        }
    }
    if ((spec.id == "codewhale" && companion.is_some()) || (spec.id == "claude" && executable.is_some())) && !probe_succeeded(&doctor) {
        issues.push(format!("{} doctor reported a problem: {}", spec.title, probe_failure_detail(&doctor)));
    }
    ...
    if spec.id == "codewhale" {
        if doctor_summary.get("status").and_then(Value::as_str).is_some_and(|status| status.eq_ignore_ascii_case("error")) {
            issues.push(format!("CodeWhale doctor JSON reported error{}", doctor_summary.get("errorKind").and_then(Value::as_str).map(|kind| format!(": {kind}")).unwrap_or_default()));
        }
    ...
    if matches!(spec.id, "codewhale" | "codex" | "opencode") && executable.is_some() && !probe_succeeded(&auth) {
        ...
        warnings.push(format!("{} authentication status could not be confirmed: {}{}", spec.title, probe_failure_detail(&auth), qualification));
    }
    ...
    let config = if spec.id == "codewhale" {
        ...
        let hook_block = config_raw.as_deref().is_some_and(hook_install::codewhale_config_has_owned_block);
        ...
        // the user reported.
        let stale_hooks: Vec<String> = config_raw.as_deref().map(|raw| {
        ...
        if let Some(models) = compatibility.get("legacyModelIds").and_then(Value::as_array).filter(|models| !models.is_empty()) {
            let names = models.iter().filter_map(Value::as_str).collect::<Vec<_>>().join(", ");
            warnings.push(format!("Legacy CodeWhale model id detected ({names}); run doctor/models because direct DeepSeek routes retired these ids while provider-owned routes may keep different mappings"));
        }
        if compatibility.get("deprecatedTlsBypass").and_then(Value::as_bool).unwrap_or(false) {
            warnings.push("CodeWhale config contains insecure_skip_tls_verify=true; current clients reject this legacy setting and require a trusted SSL_CERT_FILE bundle".into());
        }
    ...
    let aider_summary = if spec.id == "aider" {
        ...
        let has_config = summary.get("configCandidates").and_then(Value::as_array).is_some_and(|items| items.iter().any(|item| item.get("present").and_then(Value::as_bool).unwrap_or(false)));
        let has_credential_env = summary.get("credentialEnvironment").and_then(Value::as_array).is_some_and(|items| !items.is_empty());
    ...
    let path_entry_count = std::env::var_os("PATH").map(|raw| std::env::split_paths(&raw).count()).unwrap_or(0);
    Ok(json!({
        "provider": spec.id,
...
fn open_gui_application(executable: &Path) -> Result<(), String> {
    #[cfg(windows)]
    {
        ...
        if is_windows_script(executable) {
            ...
            crate::platform::hide_console_window(&mut command);
            return command.spawn().map(|_| ()).map_err(|e| format!("failed to launch {}: {e}", executable.display()));
        }
    }
    let mut command = Command::new(executable);
    // R22: hide console window for direct-exec GUI launches.
    crate::platform::hide_console_window(&mut command);
    command.spawn().map(|_| ()).map_err(|e| format!("failed to launch {}: {e}", executable.display()))
}
...
#[tauri::command]
pub fn primary_action(app: AppHandle, state: State<'_, AppState>, window: tauri::WebviewWindow) -> Result<(), String> {
    let stats = state.runtime.stats();
    let active_session = stats.get("sessions").and_then(Value::as_array).and_then(|sessions| sessions.iter().find(|session| session.get("status").and_then(Value::as_str).is_some_and(|status| !matches!(status, "ended" | "idle")))).and_then(|session| session.get("id").and_then(Value::as_str)).map(str::to_owned);
    if let Some(session_id) = active_session {
        ...
        if !providers.is_empty() {
            let _ = window.emit("pet:event", json!({"kind":"choose-provider","providers":providers}));
            Ok(())
        } else {
            open_panel(app)
        }
...
fn is_windows_script(path: &Path) -> bool {
    path.extension().and_then(|value| value.to_str()).is_some_and(|value| value.eq_ignore_ascii_case("cmd") || value.eq_ignore_ascii_case("bat"))
}
...
fn launch_terminal(spec: AgentSpec, executable: &Path, cwd: &Path) -> Result<(), String> {
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(wt) = which("wt.exe").or_else(|| which("wt")) {
            ...
            command.args(["-w", "-1", "new-tab", "--title", spec.title, "--suppressApplicationTitle", "--startingDirectory"]).arg(cwd);
            if is_windows_script(executable) {
                ...
                use std::os::windows::process::CommandExt;
                command.arg("cmd.exe").args(["/D", "/S", "/K"]).raw_arg(cmd_launch_call(executable, launch_args));
            } else {
        ...
        command.current_dir(cwd).spawn().map(|_| ()).map_err(|e| format!("failed to launch {} in Windows Terminal or cmd.exe: {e}", spec.title))
    }
    #[cfg(target_os = "macos")]
    {
        ...
        let shell = format!("cd {} && exec {}", shell_quote(&cwd.to_string_lossy()), command.join(" "));
    ...
    {
        let provider_args = agent_launch_args(spec).iter().map(|arg| OsString::from(*arg)).collect::<Vec<_>>();
...
#[cfg(test)]
mod tests {
    use super::{logical_to_physical, place_pet_anchor, PetAnchorGeometry, PetPlacement, PetWorkArea};

    const WORK: PetWorkArea = PetWorkArea {
        x: 0,
        y: 0,
        width: 1920,
        height: 1080,
    ...
    fn placement(old_x: i64, old_y: i64, width: u32, height: u32) -> super::PetPlacement {
        place_pet_anchor(PetAnchorGeometry {
            old_x,
            old_y,
            old_width: 320,
            old_height: 340,
    ...
    fn anchor_of(placement: super::PetPlacement) -> (i64, i64) {
        (
            placement.x + i64::from(placement.width) / 2 + placement.anchor_shift_x,
            placement.y + i64::from(placement.height) + placement.anchor_shift_y,
        )
    }

    fn assert_inside(placement: PetPlacement, work: PetWorkArea) {
        assert!(placement.x >= work.x && placement.y >= work.y);
        assert!(placement.x + i64::from(placement.width) <= work.x + i64::from(work.width));
        assert!(placement.y + i64::from(placement.height) <= work.y + i64::from(work.height));
    }

    #[test]
    fn ordinary_anchor_is_stable() {
        assert_eq!(anchor_of(placement(500, 400, 520, 620)), (660, 740));
    }

    #[test]
    fn right_edge_anchor_is_stable() {
        let placed = placement(1600, 400, 520, 620);
        assert_eq!(anchor_of(placed), (1760, 740));
        assert_eq!(placed.x, 1400, "HUD expands leftward without moving the pet");
        assert_eq!(placed.anchor_shift_x, 100);
    }

    #[test]
    fn right_edge_layout_stays_in_work_area() {
        assert_inside(placement(1600, 400, 520, 620), WORK);
    }

    #[test]
    fn left_edge_layout_stays_in_work_area() {
        let placed = placement(0, 400, 520, 620);
        assert_eq!(anchor_of(placed), (160, 740));
        assert_inside(placed, WORK);
    }

    #[test]
    fn bottom_edge_anchor_is_stable() {
        let placed = placement(500, 740, 520, 620);
        assert_eq!(anchor_of(placed), (660, 1080));
        assert_eq!(placed.y, 460, "HUD expands upward without moving the pet");
    }

    #[test]
    fn top_edge_layout_stays_in_work_area() {
        let placed = placement(500, 0, 520, 620);
        assert_eq!(anchor_of(placed), (660, 340));
        assert_inside(placed, WORK);
    }

    #[test]
    fn corner_anchor_is_stable() {
        let placed = placement(1600, 740, 520, 620);
        assert_eq!(anchor_of(placed), (1760, 1080));
        assert_eq!((placed.x, placed.y), (1400, 460));
    }

    #[test]
    fn opposite_corner_layout_stays_in_work_area() {
        let placed = placement(0, 0, 520, 620);
        assert_eq!(anchor_of(placed), (160, 340));
        assert_inside(placed, WORK);
    }

    #[test]
    fn negative_origin_scaled_layout_stays_in_work_area() {
        let scale = 1.5;
        let work = PetWorkArea { x: -1920, y: -120, width: 1920, height: 1080 };
        let placed = place_pet_anchor(PetAnchorGeometry {
            old_x: -480,
            old_y: 450,
            old_width: logical_to_physical(320.0, scale),
            old_height: logical_to_physical(340.0, scale),
            target_width: logical_to_physical(520.0, scale),
            target_height: logical_to_physical(620.0, scale),
            work_area: Some(work),
        });
        assert_eq!(anchor_of(placed), (-240, 960));
        assert_inside(placed, work);
    }
}
