...
fn place_pet_anchor(geometry: PetAnchorGeometry) -> PetPlacement {
    ...
    let (x, y) = if let Some(work) = geometry.work_area {
        ...
        (
            anchor_x - i64::from(width) / 2,
            anchor_y - i64::from(height),
        )
    };
    let anchor_shift_x = anchor_x - (x + i64::from(width) / 2);
    let anchor_shift_y = anchor_y - (y + i64::from(height));
    let side = if anchor_shift_x > 0 {
        "right"
    } else if anchor_shift_x < 0 {
        "left"
    } else if anchor_shift_y > 0 {
        "bottom"
    } else if anchor_shift_y < 0 {
        "top"
    } else {
...
fn run_diagnostic_probe_capture(
    ...
    if !control.register_pid(pid) {
        ...
        let _ = stdout_reader
            .join()
            .map_err(|e| eprintln!("stdout reader thread panicked: {e:?}"));
        let _ = stderr_reader
            .join()
            .map_err(|e| eprintln!("stderr reader thread panicked: {e:?}"));
        return cancelled_probe_capture();
    }
...
mod tests {
    use super::{
        logical_to_physical, place_pet_anchor, PetAnchorGeometry, PetPlacement, PetWorkArea,
    };
    ...
    fn right_edge_anchor_is_stable() {
        ...
        assert_eq!(
            placed.x, 1400,
            "HUD expands leftward without moving the pet"
        );
