...
// Install only observer-safe lifecycle events; decision/replacement and
// content-bearing hooks (ConfigChange, UserPromptExpansion, WorktreeCreate,
// FileChanged, MessageDisplay, PostToolBatch) are excluded for privacy/size.
...
fn cleanup_provider_with_path(id: &str, receipt_path: Option<&Path>) -> CleanupResult {
    match id {
        "claude" => {
            // Claude's path is always ~/.claude/settings.json; use receipt path if available.
...
fn install_codewhale(runtime: &Runtime) -> Result<InstallResult, String> {
    ...
    // tables remain diagnostics-only. Backup failure is fail-closed.
    let backup_path: Option<PathBuf> = if path.exists() {
        match backup_codewhale_config(&path, runtime) {
            Ok(p) => p,
            Err(err) => {
                return Err(format!(
                    "CodeWhale pre-write backup failed — aborting install to protect existing config: {err}. Check disk space, permissions, and antivirus locking. No changes were made."
                ));
            }
        }
    } else {
...
fn install_aider(runtime: &Runtime) -> Result<InstallResult, String> {
    ...
    let current = match fs::read_to_string(&path) {
        ...
        Err(e) => return Err(format!("Aider config exists but cannot be read ({e}). Aborting to protect existing config. No changes were made.")),
    };
    let stripped = strip_marker_variants(&current, AIDER_MARKERS)?;
    // Detect both underscore (config) and hyphen (CLI flag) forms of
...
fn ensure_codewhale_hooks_enabled(input: &str) -> String {
    // CodeWhale ignores [[hooks.hooks]] entries unless the global [hooks].enabled
    // switch is true; touch only the exact top-level table.
