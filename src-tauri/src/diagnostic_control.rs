...
impl DiagnosticControl {
    ...
    /// Claims responsibility for terminating the current child. Exactly one
    /// caller can claim a PID, preventing concurrent taskkill/killpg calls.
    ...
    pub(crate) fn restore_pid_after_failed_termination(&self, pid: u32) {
        let mut state = self.state.lock().unwrap_or_else(|error| error.into_inner());
        if state.provider.is_some() && state.cancel_requested && state.pid.is_none() {
            state.pid = Some(pid);
        }
    }
    ...
    pub(crate) fn request_cancel(&self) -> CancelRequest {
        ...
        let pid = if active { state.pid.take() } else { None };
        CancelRequest { active, pid }
    }
...
mod tests {
    ...
    #[test]
    fn cancel_returns_the_current_pid_and_clear_is_generation_safe() {
        let control = DiagnosticControl::default();
        control.begin("opencode".into()).unwrap();
        assert!(control.register_pid(41));
        control.clear_pid(99);
        assert_eq!(control.request_cancel().pid, Some(41));
        assert!(!control.claim_pid_for_termination(41));
        control.restore_pid_after_failed_termination(41);
        assert!(control.claim_pid_for_termination(41));
        assert_eq!(control.request_cancel().pid, None);
    }
}
